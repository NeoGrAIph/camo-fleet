...
| `RUNNER_DISABLE_HTTP3` | `true` | Полностью отключает HTTP/3 в Firefox (`network.http.http3.enabled`, `network.http.http3.enable_0rtt`, `network.http.http3.alt_svc`, `network.http.http3.retry_different_host`, `MOZ_DISABLE_HTTP3`), чтобы избежать ошибок TLS (`PR_END_OF_FILE_ERROR`) в средах без поддержки UDP/QUIC. |
