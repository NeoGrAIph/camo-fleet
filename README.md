...
## Документация по VNC/noVNC маршруту

Полное описание пути трафика от браузера пользователя до целевого VNC-сервера, включая конфигурацию
Traefik, websockify и docker-compose окружение, доступно в каталоге
[`docs/novnc`](docs/novnc/README.md). Там же находятся артефакты: sequence-диаграмма
(`flow.mmd`), таблица маршрутов (`routes.csv`), обзор переменных окружения (`env.md`), выдержки из
конфигураций (`reverse-proxy.md`) и набор кодовых примеров (`code-snippets.md`). При обновлении или
диагностике VNC-маршрута проверяйте этот раздел в первую очередь.
