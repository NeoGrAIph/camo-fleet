...
- **`namespace`** – keep `camofleet` unless you have a naming convention in your cluster. Оставьте `create: false`, потому что 
  Helm создаст namespace автоматически благодаря флагу `--create-namespace`. Включайте `create: true` только если удаляете флаг
  и точно знаете, что namespace ещё не создан.
...
1. Скопируйте скрипт и сохраните его, например, в `deploy/helm/install.sh`.
  ...
  HELM_ARGS=(
    --namespace "${NAMESPACE}"
    --create-namespace
    --set-string "images.registry=${IMAGE_REGISTRY}"
  )
...
2. Отредактируйте блок «настройте под себя»: укажите реестр, пространство имён и пути к Dockerfile. Если вы используете собственный приватный реестр k3s (`k3s ctr images import`), поменяйте `CONTAINERD_REF_PREFIX` и при необходимости задайте отличный от него `IMAGE_REGISTRY` (значение попадёт в `.Values.images.registry`).
